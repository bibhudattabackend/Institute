...
import { instituteLogoUpload, studentPhotoUpload } from "../uploadMiddleware.js";
import {
  isCloudinaryEnabled,
  uploadInstituteLogoBuffer,
  uploadStudentPhotoBuffer,
} from "../cloudinaryPhotos.js";

export const uploadRouter = Router();
const upload = studentPhotoUpload();
const logoUpload = instituteLogoUpload();
...
uploadRouter.post("/institute-logo", authRequired, logoUpload.single("logo"), async (req, res, next) => {
  try {
    if (!isCloudinaryEnabled()) {
      return res.status(503).json({
        error:
          "Logo upload requires CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY, and CLOUDINARY_API_SECRET in .env",
      });
    }
    if (!req.file?.buffer) {
    ...
    const instituteId = req.institute.id;
    const logo_url = await uploadInstituteLogoBuffer(req.file.buffer, req.file.mimetype, instituteId);
    return res.json({ logo_url });
  } catch (err) {
    console.error("Institute logo upload failed:", err);
    next(err);
  }
});
